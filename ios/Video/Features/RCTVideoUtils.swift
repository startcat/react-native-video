...
enum RCTVideoUtils {
     ...
     */
    static func calculatePlayableDuration(_ player: AVPlayer?, withSource source: VideoSource?)
        -> NSNumber
    {
        guard let player,
        ...
        if source?.cropStart != nil && source?.cropEnd != nil {
            return NSNumber(
                value: (Float64(source?.cropEnd ?? 0) - Float64(source?.cropStart ?? 0)) / 1000)
        }
        ...
        if let effectiveTimeRange {
            ...
            if playableDuration > 0 {
                if source?.cropStart != nil {
                    return NSNumber(
                        value: playableDuration - Float64(source?.cropStart ?? 0) / 1000)
                }
    ...
    static func urlFilePath(filepath: NSString!, searchPath: FileManager.SearchPathDirectory)
        -> NSURL!
    {
    ...
    static func getAudioTrackInfo(_ player: AVPlayer?) async -> [AnyObject] {
        ...
        for i in 0..<(group?.options.count ?? 0) {
            ...
            let selectedOption: AVMediaSelectionOption? = player.currentItem?.currentMediaSelection
    ...
    static func extractDataFromCustomSchemeUrl(from url: URL, scheme: String) -> Data? {
        guard url.scheme == scheme,
            let adoptURL = RCTVideoUtils.replaceURLScheme(url: url, scheme: nil)
        else { return nil }
    ...
    static func getValidTextTracks(
        asset: AVAsset, assetOptions: NSDictionary?, mixComposition: AVMutableComposition,
        ...
        if let textTracks, !textTracks.isEmpty {
            for textTrack in textTracks {
                ...
                if textUri.lowercased().hasPrefix("http") {
                    ...
                    let searchPath: FileManager.SearchPathDirectory =
                        isDisabledTrack ? .cachesDirectory : .documentDirectory
                    textURLAsset = AVURLAsset(
                        url: RCTVideoUtils.urlFilePath(
                            filepath: textUri as NSString?, searchPath: searchPath) as URL,
                        options: nil
                    )
                }

                if let track = await RCTVideoAssetsUtils.getTracks(
                    asset: textURLAsset, withMediaType: .text)
                {
            ...
            for (index, tracksPair) in tracks.enumerated() {
                ...
                let textCompTrack: AVMutableCompositionTrack! = mixComposition.addMutableTrack(
                    withMediaType: AVMediaType.text,
                    preferredTrackID: kCMPersistentTrackID_Invalid)

                do {
                    try textCompTrack.insertTimeRange(
                        CMTimeRangeMake(start: .zero, duration: trackAsset.duration), of: track,
                        at: .zero)
                    validTextTracks.append(textTracks[index])
                } catch {
                    // TODO: upgrade error by call some props callback to better inform user
                    print(
                        "Error occurred on textTrack insert attempt: \(error.localizedDescription)")
                    continue
                }
            }
        }
        ...
        if !fileManager.fileExists(atPath: filePath) {
            ...
            do {
                try stringToWrite.write(
                    to: URL(fileURLWithPath: filePath), atomically: true,
    ...
    static func preparePHAsset(uri: String) async -> AVAsset? {
        ...
        guard
            let phAsset = PHAsset.fetchAssets(withLocalIdentifiers: [assetId], options: nil)
                .firstObject
        else {
            return nil
        }
        ...
        return await withCheckedContinuation { continuation in
            PHCachingImageManager().requestAVAsset(forVideo: phAsset, options: options) {
                data, _, _ in
    ...
    static func prepareAsset(source: VideoSource) -> (
        asset: AVURLAsset?, assetOptions: NSMutableDictionary?
    )? {
        var asset: AVURLAsset!
        let bundlePath = Bundle.main.path(forResource: source.uri, ofType: source.type) ?? ""
        let encodedUri = source.uri?.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)
        let url =
            (source.isNetwork || source.isAsset)
            ? URL(string: encodedUri ?? "")
            : URL(fileURLWithPath: bundlePath)
        let assetOptions: NSMutableDictionary! = NSMutableDictionary()

        if source.isNetwork {
            if let headers = source.requestHeaders, !headers.isEmpty {
                assetOptions.setObject(
                    headers, forKey: "AVURLAssetHTTPHeaderFieldsKey" as NSCopying)
            }
    ...
    static func createMetadataItem(
    ...
    static func generateVideoComposition(asset: AVAsset, filter: CIFilter) async
        -> AVVideoComposition?
    {
            ...
            return try? await AVVideoComposition.videoComposition(
                ...
                applyingCIFiltersWithHandler: { (request: AVAsynchronousCIImageFilteringRequest) in
                    if filter == nil {
                        ...
                        let output: CIImage! = filter.outputImage?.cropped(
                            to: request.sourceImage.extent)
                ...
                return AVVideoComposition(
                    ...
                    applyingCIFiltersWithHandler: {
                        (request: AVAsynchronousCIImageFilteringRequest) in
                        if filter == nil {
                            request.finish(with: request.sourceImage, context: nil)
                        } else {
                            let image: CIImage! = request.sourceImage.clampedToExtent()
