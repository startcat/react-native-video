...
class ContentKeyManager: NSObject, AVContentKeySessionDelegate {
    ...
    */
    func contentKeySession(_ session: AVContentKeySession, didProvide keyRequest: AVContentKeyRequest) {
        RCTLog("[Native Downloads] (ContentKeyManager) Content is encrypted. Initiating key request")
    ...
    func contentKeySession(_ session: AVContentKeySession, didProvideRenewingContentKeyRequest keyRequest: AVContentKeyRequest) {
        RCTLog("[Native Downloads] (ContentKeyManager) Renewal of an existing content key")
    ...
    func handleOnlineContentKeyRequest(keyRequest: AVContentKeyRequest) {
        if self.fpsCertificate == nil {
            RCTLog("[Native Downloads] (ContentKeyManager) Application Certificate missing, will request")
            
            // Request Application Certificate
            do {
                try self.requestApplicationCertificate()
            } catch {
                RCTLog("[Native Downloads] (ContentKeyManager) Failed requesting Application Certificate: \(error)")
                return
            }
        }
        ...
        */
        let contentIdentifierData = contentIdentifier.data(using: .utf8) else {
           RCTLog("[Native Downloads] (ContentKeyManager) ERROR: Failed to retrieve the contentIdentifier from the keyRequest!")
           return
        }
        ...
        */
        ...
        RCTLog("[Native Downloads] (ContentKeyManager) Key request info:\n \(contentKeyIdAndIv)")
        ...
        if downloadRequestedByUser || persistableContentKeyExistsOnDisk(withAssetName: asset.name, withContentKeyIV: keyIV) || shouldRequestPersistableContentKey(withIdentifier: contentKeyIdentifierString) {
            ...
            */
            do {
                ...
                RCTLog("[Native Downloads] (ContentKeyManager) WARNING: User requested offline capabilities for the asset. But key loading request from an AirPlay Session requires online key")
                /*
    ...
    func provideOnlineKey(withKeyRequest keyRequest: AVContentKeyRequest, contentIdentifier contentIdentifierData: Data) {
        ...
        */
        let getCkcAndMakeContentAvailable = { [weak self] (spcData: Data?, error: Error?) in
            ...
            if let error = error {
                RCTLog("[Native Downloads] (ContentKeyManager) ERROR: Failed to prepare SPC: \(error)")
                /*
            ...
            do {
                RCTLog("[Native Downloads] (ContentKeyManager) Will use SPC (Server Playback Context) to request CKC (Content Key Context) from KSM (Key Security Module)")
                ...
                */
                ...
                RCTLog("[Native Downloads] (ContentKeyManager) Creating Content Key Response from CKC obtaned from Key Server")
                ...
                */
                ...
                RCTLog("[Native Downloads] (ContentKeyManager) Failed to make protected content available for processing: \(error)")
        ...
        RCTLog("[Native Downloads] (ContentKeyManager) Will prepare content key request SPC (Server Playback Context)")
    ...
    */
    func requestPersistableContentKeys(forAsset asset: Asset) {
        RCTLog("[Native Downloads] (ContentKeyManager) OFFLINE KEY FLOW")
        
        for contentKeyId in asset.contentKeyIdList ?? [] {
            RCTLog("[Native Downloads] (ContentKeyManager) Initiating Persistable Key Request for key identifier: \(String(describing: contentKeyId))")
    ...
    */
    func contentKeySession(_ session: AVContentKeySession, didProvide keyRequest: AVPersistableContentKeyRequest) {
        RCTLog("[Native Downloads] (ContentKeyManager) Initiating persistable key request")
        ...
        let completionHandler = { [weak self] (spcData: Data?, error: Error?) in
            ...
            do {
                ...
                */
                ...
                RCTLog("[Native Downloads] (ContentKeyManager) Persistable Content Key was obtained from Content Key Context (CKC)")
                ...
                */
                ...
                RCTLog("[Native Downloads] (ContentKeyManager) ERROR: \(error)")
        ...
        */
        if persistableContentKeyExistsOnDisk(withAssetName: asset.name, withContentKeyIV: keyIV) {
            ...
            guard let contentKey = FileManager.default.contents(atPath: urlToPersistableKey.path) else {
                ...
                RCTLog("[Native Downloads] (ContentKeyManager) Failed to locate Presistable key from disk. Attempting to create a new one")
            ...
            /*
             ...
             decrypting content.
             */
            RCTLog("[Native Downloads] (ContentKeyManager) Creating Content Key Response from persistent CKC")
    ...
    */
    func contentKeySession(_ session: AVContentKeySession,
        ...
        RCTLog("[Native Downloads] (ContentKeyManager) Updating Persistable Content Key")
        ...
        do {
            ...
            */
            let contentIdentifier = contentKeyIdentifierString.replacingOccurrences(of: "skd://", with: "") as String?
               ...
               RCTLog("[Native Downloads] (ContentKeyManager) ERROR: Failed to retrieve the contentIdentifier")
               return
            }
    ...
    func writePersistableContentKey(contentKey: Data, withAssetName assetName: String, withContentKeyIV keyIV: String) throws {
        ...
        RCTLog("[Native Downloads] (ContentKeyManager) Wrote persistable content key to disk for \(assetName) to location: \(fileURL)")
    }
    ...
    func deletePeristableContentKey(withAssetName assetName: String, withContentKeyId keyId: String) {
        ...
        */
        ...
        if persistableContentKeyExistsOnDisk(withAssetName: assetName, withContentKeyIV: keyIV) {
            ...
            RCTLog("[Native Downloads] (ContentKeyManager) Deleting content key for \(assetName) - \(keyIV): No persistable content key exists on disk")
            return
        }
        ...
        do {
            ...
            RCTLog("[Native Downloads] (ContentKeyManager) Presistable Key for \(assetName)-\(keyIV) was deleted")
        } catch {
            RCTLog("[Native Downloads] (ContentKeyManager) An error occured removing the persisted content key: \(error)")
        }
    }
    
    func requestApplicationCertificate() throws {
        RCTLog("[Native Downloads] (ContentKeyManager) Requesting FPS Certificate")
    
        guard let url = URL(string: fpsCertificateUrl) else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: missingApplicationCertificateUrl")
        ...
        if let error = error {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: Error getting FPS Certificate: \(error)")
            throw ProgramError.applicationCertificateRequestFailed
        }
        guard response != nil else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: FPS Certificate request response empty")
            throw ProgramError.applicationCertificateRequestFailed
        }
        guard data != nil else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: FPS Certificate request response data is empty")
            throw ProgramError.applicationCertificateRequestFailed
        }
        ...
        guard certificate != nil else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: FPS Certificate data is not a valid DER-encoded")
            throw ProgramError.applicationCertificateRequestFailed
        }
        ...
        if let certificate = certificate {
            ...
            if let summary = summary {
                RCTLog("[Native Downloads] (ContentKeyManager) FPS Certificate received, summary: \(summary)")
            }
        }
    }
    ...
    */
    ...
    func requestContentKeyFromKeySecurityModule(spcData: Data) throws -> Data {
        ...
        guard let url = URL(string: licensingServiceUrl) else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: missingLicensingServiceUrl")
        ...
        */
        ...
        ksmRequest.setValue("", forHTTPHeaderField: "X-AxDRM-Message")
        ksmRequest.httpBody = spcData
        
        let (data, response, error) = URLSession.shared.synchronousDataTask(urlRequest: ksmRequest)
        
        if let error = error {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: Error getting CKC: \(error)")
            throw ProgramError.noCKCReturnedByKSM
        }
        guard response != nil else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: CKC request response empty")
            throw ProgramError.noCKCReturnedByKSM
            
        }
        guard data != nil else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: CKC response data is empty")
            throw ProgramError.noCKCReturnedByKSM
        }
        
        RCTLog("[Native Downloads] (ContentKeyManager) SUCCESS Requesting Content Key Context (CKC) from Key Security Module (KSM)")
        ...
        if let httpUrlResponse = response as? HTTPURLResponse {
            ...
            """
            RCTLog("[Native Downloads] (ContentKeyManager) CKC response custom headers:\n \(CKCResponseString)")
        }
        
        ckcData = data

        guard ckcData != nil else {
            RCTLog("[Native Downloads] (ContentKeyManager) ERROR: No CKC returned By KSM")
            throw ProgramError.noCKCReturnedByKSM
        }
