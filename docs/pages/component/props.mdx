...
On Android the exit from picture in picture is user-driven (expand or close the PiP window): setting the prop back to `false` is a no-op there, and the state is reported through `onPictureInPictureStatusChanged`. The host activity must declare `android:supportsPictureInPicture="true"` in its manifest.
...
Determine whether the media should automatically enter picture in picture when the user leaves the app while playing (home gesture / app switch).

- **false (default)** - Don't enter picture in picture automatically
- **true** - Enter picture in picture automatically when leaving the app while playing

On iOS this relies on `canStartPictureInPictureAutomaticallyFromInline` (iOS 14.2+) and requires the "Audio, AirPlay and Picture in Picture" background mode. On Android it uses `setAutoEnterEnabled` (API 31+) with an `onUserLeaveHint` fallback for API 26-30, and requires `android:supportsPictureInPicture="true"` on the host activity.
...
**For Android**, the foreground service and the required `FOREGROUND_SERVICE` / `FOREGROUND_SERVICE_MEDIA_PLAYBACK` permissions are provided automatically by the [`@overon/react-native-overon-player-now-playing`](https://gitlab.overon.es/ott/modules/player/react-native-overon-player-now-playing) module via autolinking (its `MediaPlaybackService` is merged into your app's manifest). No manual `AndroidManifest.xml` changes are required.
