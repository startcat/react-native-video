...
>((props: ReactNativeVideoAdapterProps, ref: React.Ref<ReactNativeVideoAdapterRef>) => {
		...
		(): PlayerAdapter => ({
			...
			async setAudioTrack(trackId: number): Promise<boolean> {
				try {
					...
					const track =
						availableAudioTracks.find((t: TrackInfo) => t.id === trackId) || null;
			...
			async setVideoTrack(trackId: number): Promise<boolean> {
				try {
					...
					const track =
						availableVideoTracks.find((t: TrackInfo) => t.id === trackId) || null;
			...
			async setTextTrack(trackId: number): Promise<boolean> {
				try {
					...
					const track =
						availableTextTracks.find((t: TrackInfo) => t.id === trackId) || null;
			...
			async setPreferredAudioLanguage(language: string): Promise<boolean> {
				...
				const preferredTrack = availableAudioTracks.find(
					(track: TrackInfo) => track.language === language
				);
	...
	// Eventos del Video
	const handleLoad = (
		e: OnLoadData & { audioTracks?: any[]; videoTracks?: any[]; textTracks?: any[] }
	) => {
		...
		if (videoTracks.length > 0) {
			setCurrentVideoTrack(videoTracks[0]);
		}
	...
	return props.contentInfo?.source ? (
		<Video
			...
			bufferConfig={
				props.contentInfo.config?.bufferConfig || {
					minBufferMs: 15000,
					maxBufferMs: 50000,
