...
| Campo               | Tipo                    | SDK Nativa                 | Descripción                                | Ejemplo                        |
| ------------------- | ----------------------- | -------------------------- | ------------------------------------------ | ------------------------------ |
| `transactionCode`   | string                  | `contentTransactionCode`   | Código único de transacción para la sesión | `"txn_abc123"`                 |
| `id`                | string                  | `contentId`                | Identificador único del contenido          | `"video_12345"`                |
| `type`              | string                  | `contentType`              | Tipo de contenido                          | `"video"`, `"audio"`, `"live"` |
| `title`             | string                  | `contentTitle`             | Título del contenido                       | `"Episodio 1: El Comienzo"`    |
| `program`           | string                  | `program`                  | Nombre del programa o serie                | `"Mi Serie Favorita"`          |
| `isLive`            | boolean                 | `live`                     | Indica si es contenido en directo          | `true` / `false`               |
| `playbackType`      | string                  | `contentPlaybackType`      | Tipo de reproducción                       | `"vod"`, `"live"`, `"dvr"`     |
| `tvShow`            | string                  | `contentTvShow`            | Nombre del programa de TV                  | `"Game of Thrones"`            |
| `season`            | string                  | `contentSeason`            | Temporada del contenido                    | `"Temporada 1"`                |
| `episodeTitle`      | string                  | `contentEpisodeTitle`      | Título específico del episodio             | `"Winter is Coming"`           |
| `channel`           | string                  | `contentChannel`           | Canal o plataforma de distribución         | `"HBO"`, `"Netflix"`           |
| `customDimension`   | YouboraCustomDimensions | `contentCustomDimension`   | Dimensiones personalizadas para análisis   | Ver tabla siguiente            |
| `saga`              | string                  | `contentSaga`              | Saga o colección de películas relacionadas | `"Star Wars"`, `"Marvel"`      |
| `subtitles`         | string                  | `contentSubtitles`         | Subtítulos seleccionados                   | `"Español"`, `"English"`       |
| `drm`               | string                  | `contentDrm`               | Tecnología DRM usada                       | `"Widevine"`, `"FairPlay"`     |
| `language`          | string                  | `contentLanguage`          | Idioma del contenido                       | `"es"`, `"en"`                 |
| `streamingProtocol` | string                  | `contentStreamingProtocol` | Protocolo de streaming                     | `"HLS"`, `"DASH"`              |
| `metadata`          | YouboraContentMetadata  | `contentMetadata`          | Metadata adicional clave:valor             | Ver tabla siguiente            |

### Metadata del contenido (YouboraContentMetadata)

El campo `metadata` permite enviar información adicional del contenido como pares clave:valor. Según la documentación oficial de NPAW:

> _"Object containing mixed extra information about the content like director, parental rating, device info or the audio channels. This object may store any serializable key:value info."_
...
if (format === "mobile") {
	return {
		...
		contentLanguage: data?.content?.language,

		// Nuevos campos de contenido
		contentSaga: data?.content?.saga,
		contentSubtitles: data?.content?.subtitles,
		contentDrm: data?.content?.drm,
		contentStreamingProtocol: data?.content?.streamingProtocol,
		contentMetadata: data?.content?.metadata,
...
if (format === "cast") {
	return {
		...
		"content.language": data?.content?.language,

		// Nuevos campos de contenido
		"content.saga": data?.content?.saga,
		"content.subtitles": data?.content?.subtitles,
		"content.drm": data?.content?.drm,
		"content.streamingProtocol": data?.content?.streamingProtocol,
		// Para metadata en cast, se mapea cada clave individualmente
		...Object.entries(data?.content?.metadata || {}).reduce(
			(acc, [key, value]) => {
				acc[`content.metadata.${key}`] = value;
				return acc;
			},
			{}
		),
