...
export class MediaFlowStateManager {
    ...
    transitionTo(
        ...
        const newState: MediaFlowState = {
            type: newStateType,
            mediaType: options.mediaType !== undefined ? options.mediaType : this.currentState.mediaType,
            ...
            metadata: {
                isAutoNext: options.isAutoNext !== undefined ? options.isAutoNext : this.currentState.metadata.isAutoNext,
                hasPlayedTudum: this.calculateHasPlayedTudum(newStateType),
                startPosition: options.startPosition !== undefined ? options.startPosition : this.currentState.metadata.startPosition,
                error: options.error
            },
            timestamp: Date.now()
        };
    ...
    private calculateHasPlayedTudum(newStateType: MediaFlowStateType): boolean {
        ...
        if (this.currentState.type === MediaFlowStateType.PLAYING_TUDUM &&
