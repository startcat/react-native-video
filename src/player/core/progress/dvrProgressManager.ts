...
export class DVRProgressManagerClass extends BaseProgressManager {
	...
	goToProgramStart(): void {
		if (!this._isValidState() || !this._currentProgram) {
			this._currentLogger?.warn("goToProgramStart: Invalid state or no program", {
				isValidState: this._isValidState(),
				hasCurrentProgram: !!this._currentProgram,
			});
			return;
		}

		const programStartTimestamp = this._currentProgram.startDate;
		const windowStart = this._getWindowStart();
		const liveEdge = this._getCurrentLiveEdge();
		const currentProgress = this._getProgressDatum();

		this._currentLogger?.info(`🎯 goToProgramStart - Program: "${this._currentProgram.title}"`);
		this._currentLogger?.info(`📊 Timestamps:`, {
			programStart: programStartTimestamp,
			programStartTime: new Date(programStartTimestamp).toLocaleString("es-ES"),
			programEnd: this._currentProgram.endDate,
			programEndTime: new Date(this._currentProgram.endDate).toLocaleString("es-ES"),
			windowStart: windowStart,
			windowStartTime: new Date(windowStart).toLocaleString("es-ES"),
			liveEdge: liveEdge,
			liveEdgeTime: new Date(liveEdge).toLocaleString("es-ES"),
			currentProgress: currentProgress,
			currentProgressTime: new Date(currentProgress).toLocaleString("es-ES"),
		...
		if (this._isPaused || this._isBuffering) {
			this._frozenProgressDatum = programStartTimestamp;
			this._currentLogger?.debug(
				`Updated frozen position to program start during pause: ${this._frozenProgressDatum}`
			);
		}

		const playerTime = this._timestampToPlayerTime(programStartTimestamp);
		this._currentLogger?.info(
			`🔄 Conversion result: timestamp ${programStartTimestamp} -> playerTime ${playerTime}s`
		);
	...
	private _timestampToPlayerTime(timestamp: number): number {
		this._currentLogger?.debug(`🔢 _timestampToPlayerTime called:`, {
			inputTimestamp: timestamp,
			inputTime: new Date(timestamp).toLocaleString("es-ES"),
			playbackType: this._playbackType,
		});

		// IMPORTANTE: El playerTime SIEMPRE debe ser relativo a windowStart
		// porque el player nativo interpreta la posición desde el inicio del stream DVR
		const windowStart = this._getWindowStart();
		const result = Math.max(0, (timestamp - windowStart) / 1000);

		if (this._playbackType === DVR_PLAYBACK_TYPE.PLAYLIST && this._currentProgram) {
			this._currentLogger?.debug(`📺 PLAYLIST mode conversion:`, {
				programStart: this._currentProgram.startDate,
				programStartTime: new Date(this._currentProgram.startDate).toLocaleString("es-ES"),
				windowStart: windowStart,
				windowStartTime: new Date(windowStart).toLocaleString("es-ES"),
				targetTimestamp: timestamp,
				targetTime: new Date(timestamp).toLocaleString("es-ES"),
				difference: timestamp - windowStart,
				resultSeconds: result,
			});
		} else {
			this._currentLogger?.debug(`🪟 WINDOW/PROGRAM mode conversion:`, {
				windowStart: windowStart,
				windowStartTime: new Date(windowStart).toLocaleString("es-ES"),
				difference: timestamp - windowStart,
				resultSeconds: result,
