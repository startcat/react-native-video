  ...
  const renderedTimeMarkers = useMemo(() => {
    return safeTimeMarkers.map((item: ITimeMarkers, index: number) => {
      ...
      if (item.type === TIME_MARK_TYPE.INTRO) {
        ...
        return (
          <TimeMarkButton key={index} title="Saltar intro" value={item.end} onPress={safeOnPress} />
        );
      }
...
export const TimeMarks: React.FC<TimeMarksProps> = React.memo(TimeMarksComponent);
