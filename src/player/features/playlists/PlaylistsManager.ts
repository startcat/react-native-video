...
export class PlaylistsManager {
  ...
  private async initializeNativeModule(): Promise<void> {
    try {
      ...
      if (this.nativeModule) {
        ...
        this.logger.warn(LOG_TAGS.PLAYLISTS_MANAGER, "Native module not available - JS-only mode");
      }
    } catch (error) {
      this.logger.warn(LOG_TAGS.PLAYLISTS_MANAGER, "Failed to initialize native module", error);
    }
  }
  ...
  public async setPlaylist(items: PlaylistItem[], config?: Partial<PlaylistConfig>): Promise<void> {
    ...
    try {
      ...
      this.logger.info(LOG_TAGS.PLAYLISTS_MANAGER, `Playlist set with ${this.items.length} items`);
    } catch (error) {
  ...
  public async goToIndex(
    ...
    this.logger.debug(LOG_TAGS.PLAYLISTS_MANAGER, `Moved to index ${index} (reason: ${reason})`);
  }
  ...
  private getPreviousIndex(): number {
    ...
    if (previous < 0) {
      return this.playlistConfig.repeatMode === PlaylistRepeatMode.ALL ? this.items.length - 1 : -1;
    }
  ...
  private async sendPlaylistToNative(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(LOG_TAGS.PLAYLISTS_MANAGER, "Failed to send playlist to native", error);
    }
  }
