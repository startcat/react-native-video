...
export abstract class BaseAnalyticsPluginFactory {
    ...
    static createPlugins(
        ...
        if (config.debug) {
            console.log(`[Analytics Factory] Created ${plugins.length} plugins:`, 
    ...
    static createPlugin(
        ...
        try {
            ...
            if (plugin) {
                if (debug) {
                    console.log(`[Analytics Factory] Created plugin: ${plugin.name} v${plugin.version}`);
                }
                return plugin;
            } else {
    ...
    protected static mapMediaDataToComscoreMetadata(mediaData: any): any {
        ...
        throw new Error('mapMediaDataToComscoreMetadata must be implemented in project-specific factory');
    }
  
    protected static mapMediaDataToAdobeMetadata(mediaData: any): any {
        // SOBRESCRIBIR en cada proyecto con su mapeo específico
        throw new Error('mapMediaDataToAdobeMetadata must be implemented in project-specific factory');
    }
