...
export class ConfigManager {
    ...
    public async updateConfig<K extends keyof ConfigDownloads>(
        ...
        try {
            ...
            this.currentLogger.debug(TAG, `Config updated: ${property}`, { oldValue, newValue: value });
    ...
    public async updateStreamQuality(quality: 'auto' | 'low' | 'medium' | 'high' | 'max'): Promise<void> {
            ...
            switch (property) {
                ...
                case 'streamQuality':
                    ...
                    if (!validQualities.includes(value as string)) {
                        throw new Error(`Invalid stream quality: ${value}. Valid values: ${validQualities.join(', ')}`);
                    }
                    break;
    ...
    private async loadPersistedConfig(): Promise<void> {
        try {
            ...
            this.currentLogger.warn(TAG, 'Failed to load persisted configuration, using defaults', error);
            this.currentDownloadsConfig = { ...DEFAULT_CONFIG };
        }
    }
    ...
    private async loadConfigFromPersistence(): Promise<ConfigDownloads | null> {
        try {
            ...
            if (persistedConfig) {
                ...
                this.currentLogger.debug(TAG, 'No persisted configuration found, will use defaults');
                return null;
            }
    ...
    private async persistConfig(): Promise<void> {
        try {
            ...
            this.eventEmitter.emit('config_saved', {
                config: this.currentDownloadsConfig
            });
            
            this.currentLogger.debug(TAG, 'Configuration persisted successfully');
    ...
    private setupAutoSave(): void {
        ...
        this.autoSaveTimer = setInterval(async () => {
            try {
                ...
                this.currentLogger.warn(TAG, 'Auto-save failed', error);
            }
        }, this.config.autoSaveInterval!);
        
        this.currentLogger.debug(TAG, `Auto-save configured every ${this.config.autoSaveInterval}ms`);
    }
