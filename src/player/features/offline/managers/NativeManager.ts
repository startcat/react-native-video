...
export class NativeManager {
    ...
    public async initialize(config?: Partial<NativeManagerConfig>): Promise<void> {
        if (this.isInitialized) {
            this.currentLogger.warn(TAG, 'NativeManager already initialized');
            return;
        }
    ...
    private setupNativeModule(): void {
        try {
            ...
            if (this.nativeModule) {
                ...
                this.currentLogger.warn(TAG, `Native module DownloadsModule2 not found for ${Platform.OS}`);
            }
        } catch (error) {
    ...
    public async hasDownload(downloadId: string): Promise<boolean> {
        ...
        try {
            ...
            this.currentLogger.error(TAG, `Failed to check download existence: ${downloadId}`, error);
            return false;
        }
    }
    ...
    public async setStreamQuality(quality: 'auto' | 'low' | 'medium' | 'high' | 'max'): Promise<void> {
        this.validateInitialized();
    ...
    public async setNetworkPolicy(config: { allowCellular: boolean; requireWifi?: boolean }): Promise<void> {
        this.validateInitialized();
    ...
    public async validateDownloadUri(uri: string): Promise<{ isValid: boolean; errors?: string[] }> {
        this.validateInitialized();
    ...
    public destroy(): void {
        ...
        this.eventSubscriptions.forEach((subscription, eventName) => {
            try {
                ...
                this.currentLogger.warn(TAG, `Error removing native subscription: ${eventName}`, error);
            }
        });
