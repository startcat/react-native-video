...
export class DownloadsManager {
    ...
    public async initialize(config?: Partial<DownloadsManagerConfig>): Promise<void> {
        ...
        try {
            ...
            this.state.error = error instanceof PlayerError ? error : 
                new PlayerError('DOWNLOAD_BINARY_SERVICE_INITIALIZATION_FAILED', { originalError: error });
    ...
    private setupServiceCoordination(): void {
        ...
        const downloadUnsubscriber = downloadService.subscribe('all', (data) => {
            this.handleDownloadEvent(data);
        });
    ...
    private setupGlobalPolicies(): void {
        ...
        queueManager.setMaxConcurrentDownloads(this.config.maxConcurrentDownloads);

        // Configurar políticas de red basadas en ConfigManager
        const downloadsConfig = configManager.getCurrentConfig();
        ...
        queueManager.setRetryPolicy(this.config.autoRetryEnabled, this.config.maxRetryAttempts);

        // Integrar con ProfileManager para filtrado de contenido
        if (this.config.profileManagementEnabled) {
            const activeProfile = profileManager.getActiveProfile();
            if (activeProfile) {
                this.currentLogger.debug(TAG, `Active profile: ${activeProfile.profileId}`);
            }
        }
    ...
    private handleDownloadEvent(data: any): void {
        try {
    ...
    private handleNetworkEvent(networkData: any): void {
        const { isConnected, isWifi, isCellular } = networkData;
    ...
    private handleAutoRetry(failedEventData: any): void {
        const { taskId, error, retryCount = 0 } = failedEventData;
    ...
    private enforceGlobalLimits(): void {
        ...
        if (stats.active > this.config.maxConcurrentDownloads) {
            this.currentLogger.warn(TAG, `Active downloads (${stats.active}) exceed limit (${this.config.maxConcurrentDownloads})`);
    ...
    public async addDownload(task: BinaryDownloadTask | StreamDownloadTask, type: DownloadType): Promise<string> {
        if (!this.state.isInitialized) {
        ...
        try {
            ...
            throw error instanceof PlayerError ? error : 
    ...
    public getActiveDownloads(): DownloadItem[] {
        return this.getDownloads().filter(item => 
    ...
    public updateConfig(newConfig: Partial<DownloadsManagerConfig>): void {
        ...
        if (newConfig.enableBinaryDownloads !== undefined || newConfig.enableStreamDownloads !== undefined) {
    ...
    public subscribe(event: DownloadEventType | 'all' | string, callback: (data: any) => void): () => void {
    ...
    public getSystemState() {
        return {
            ...
            services: {
                ...
                network: this.config.networkMonitoringEnabled ? networkService.getStatus() : null,
                storage: this.config.storageMonitoringEnabled ? storageService.getInfo() : null
            }
        };
    }
    ...
    private async validateGlobalPolicies(task: BinaryDownloadTask | StreamDownloadTask, type: DownloadType): Promise<void> {
