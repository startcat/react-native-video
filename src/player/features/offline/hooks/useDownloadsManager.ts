...
interface UseDownloadsManagerReturn {
    ...
    addDownload: (task: BinaryDownloadTask | StreamDownloadTask, type: DownloadType) => Promise<string>;
...
export function useDownloadsManager(options: UseDownloadsManagerOptions = {}): UseDownloadsManagerReturn {
    const {
        autoInit = true,
        config,
        onError,
    ...
    const initializeManager = useCallback(async () => {
        try {
            ...
            const error = err instanceof PlayerError ? err : 
    ...
    const activeDownloads = useMemo(() => 
        downloads.filter((item: DownloadItem) => 
