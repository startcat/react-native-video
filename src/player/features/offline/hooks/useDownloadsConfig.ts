...
export function useDownloadsConfig(): UseDownloadsConfigReturn {
    // Estados reactivos
    const [config, setConfig] = useState<ConfigDownloads>(() => 
        ...
        const initializeConfigManager = async () => {
            try {
                ...
                await configManager.initialize({
                    ...
                    autoSaveInterval: 2000 // Auto-save cada 2 segundos
                });
    ...
    useEffect(() => {
        ...
        const unsubscribeValidationFailed = configManager.subscribe('config_validation_failed', handleConfigValidationFailed);
    ...
    const updateStreamQuality = useCallback((quality: 'auto' | 'low' | 'medium' | 'high' | 'max'): void => {
...
export function useDownloadsConfigExtended(): UseDownloadsConfigExtendedReturn {
    ...
    // Estado adicional
    const [defaultConfig] = useState<ConfigDownloads>(() => 
        configManager.getConfig()
    );
    ...
    const updateLogLevel = useCallback(async (level: 'DEBUG' | 'INFO' | 'WARN' | 'ERROR'): Promise<void> => {
    ...
    const updateMultipleConfig = useCallback(async (updates: Partial<ConfigDownloads>): Promise<void> => {
