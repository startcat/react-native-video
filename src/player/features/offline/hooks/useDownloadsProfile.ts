...
export function useDownloadsProfile(): UseDownloadsProfileReturn {
    ...
    useEffect(() => {
        const unsubscribe = profileManager.subscribe(
            ProfileEventType.PROFILE_CHANGED,
            (data) => {
    ...
    const filterByActiveProfile = useCallback((downloads: DownloadItem[]): DownloadItem[] => {
    ...
    const switchProfile = useCallback((profileId: string | null) => {
        ...
        if (profileId) {
            // En una implementación real, aquí buscarías el perfil por ID
            // Por ahora, creamos un contexto básico
            profileContext = {
                id: profileId,
        ...
        profileManager.setActiveProfile(profileContext);
    }, []);
...
export function useCanDownload(): {
    ...
    useEffect(() => {
        const unsubscribe = profileManager.subscribe(
            ProfileEventType.CONFIG_CHANGED,
            () => {
    ...
    const canDownloadContent = useCallback((downloadItem: DownloadItem) => {
        try {
