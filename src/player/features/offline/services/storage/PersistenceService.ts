...
export class PersistenceService {
	...
	public async updateDownloadItem(id: string, updates: Partial<DownloadItem>): Promise<void> {
		try {
			...
			const updated: DownloadItem = {
				...
				profileIds:
					updates.profileIds !== undefined ? updates.profileIds : existing.profileIds,
				// Merge subtitles properly
				subtitles: updates.subtitles !== undefined ? updates.subtitles : existing.subtitles,
			};
	...
	public async updateDownloadsConfig(
		updates: Partial<ConfigDownloads>
	): Promise<ConfigDownloads> {
		try {
	...
	public async updateDownloadProgress(
		...
		try {
			...
			this.currentLogger.error(
				TAG,
				`Failed to update download progress: ${downloadId}`,
				error
			);
			throw new PlayerError("STORAGE_ASYNC_002", {
				originalError: error,
	...
	public async updateDownloadState(
		downloadId: string,
	...
	public async importData(data: string): Promise<void> {
		...
		try {
			...
			const downloads = new Map<string, DownloadItem>(
				migratedData.downloads as Array<[string, DownloadItem]>
			);
			await this.saveDownloadState(downloads);
		...
		try {
			...
			if (currentVersion < this.dataVersion) {
				this.currentLogger.info(
					TAG,
					`Data version mismatch. Current: ${currentVersion}, Expected: ${this.dataVersion}`
				);
