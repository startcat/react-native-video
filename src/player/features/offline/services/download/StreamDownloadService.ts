...
export class StreamDownloadService {
    ...
    public async initialize(config?: Partial<StreamDownloadServiceConfig>): Promise<void> {
        if (this.isInitialized) {
            this.currentLogger.info(TAG, 'StreamDownloadService already initialized');
            return;
        }
        ...
        try {
            ...
            for (const download of downloads) {
                if (download.type === 'STREAM') {
                    this.currentLogger.info(TAG, `Recovered stream download: ${download.id} - State: ${download.state}`);
            ...
            this.currentLogger.info(TAG, `Recovered ${this.activeDownloads.size} pending stream downloads`);
    ...
    private handleNetworkChange(networkStatus: any): void {
            ...
            for (const [downloadId, download] of this.activeDownloads) {
                if (download.state === DownloadStates.DOWNLOADING) {
                    this.pauseDownload(downloadId).catch(error => {
                        this.currentLogger.error(TAG, `Failed to pause download on network change: ${downloadId}`, error);
                    });
                }
            }
        }
        ...
        if (networkStatus.isConnected && (!this.config.requiresWifi || networkStatus.isWifi)) {
            ...
            for (const [downloadId, download] of this.activeDownloads) {
                if (download.state === DownloadStates.PAUSED || download.state === DownloadStates.WAITING_FOR_NETWORK) {
                    this.resumeDownload(downloadId).catch(error => {
                        this.currentLogger.error(TAG, `Failed to resume download on network restoration: ${downloadId}`, error);
                    });
                }
            }
        }
    ...
    private async executeStreamDownload(task: StreamDownloadTask): Promise<void> {
        ...
        try {
            ...
            this.currentLogger.info(TAG, `Stream download started: ${task.id} - ${task.manifestUrl}`);
    ...
    private async handleStreamDownloadError(downloadId: string, error: any): Promise<void> {
        ...
        if (download.retryCount < this.config.maxRetries) {
            this.currentLogger.warn(TAG, `Stream download failed, retrying (${download.retryCount}/${this.config.maxRetries}): ${downloadId}`);
    ...
    public setNetworkPolicy(requiresWifi: boolean, allowCellular: boolean): void {
        ...
        nativeManager.setNetworkPolicy({ 
            allowCellular,
    ...
    public destroy(): void {
        ...
        for (const downloadId of this.activeDownloads.keys()) {
            this.cancelDownload(downloadId).catch((error: any) => {
                this.currentLogger.error(TAG, `Failed to cancel download during destroy: ${downloadId}`, error);
            });
        }
