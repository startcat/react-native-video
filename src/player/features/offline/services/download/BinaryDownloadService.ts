...
export class BinaryDownloadService {
      ...
      for (const task of tasks) {
        ...
        const activeDownload: ActiveBinaryDownload = {
          ...
          progress: {
            ...
            percent:
              task.bytesTotal > 0 ? Math.round((task.bytesDownloaded / task.bytesTotal) * 100) : 0,
          },
        };
  ...
  public async pauseDownload(taskId: string): Promise<void> {
    ...
    try {
      // Pausar usando background downloader (solo iOS)
      download.downloadTask.pause(); // pause() es síncrono, no devuelve Promise
  ...
  public async resumeDownload(taskId: string): Promise<void> {
    ...
    try {
      ...
      // Reanudar usando background downloader (solo iOS)
      download.downloadTask.resume(); // resume() es síncrono, no devuelve Promise
    ...
    downloadTask
      .progress(
        ({ bytesDownloaded, bytesTotal }: { bytesDownloaded: number; bytesTotal: number }) => {
          ...
          this.eventEmitter.emit(DownloadEventType.PROGRESS, progress);
          this.currentLogger.debug(TAG, `Recovery progress: ${taskId} - ${progress.percent}%`);
        }
      )
      .done(() => {
