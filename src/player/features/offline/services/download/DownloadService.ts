...
export class DownloadService {
    ...
    public async initialize(config?: Partial<DownloadServiceConfig>): Promise<void> {
        if (this.isInitialized) {
            this.currentLogger.info(TAG, 'DownloadService already initialized');
            return;
        }
    ...
    public async startDownload(task: BinaryDownloadTask | StreamDownloadTask, type: DownloadType): Promise<void> {
        if (!this.isInitialized) {
    ...
    public getDownloadState(downloadId: string, type: DownloadType): ActiveBinaryDownload | ActiveStreamDownload | null {
        if (!this.isTypeEnabled(type)) {
            return null;
        }
    ...
    public getUnifiedStats() {
        ...
        stats.combined.averageSpeed = activeStrategies > 0 ? 
            (binarySpeed + streamSpeed) / activeStrategies : 0;
    ...
    public validateDownloadTask(task: BinaryDownloadTask | StreamDownloadTask, type: DownloadType): ValidationResult {
        const errors: string[] = [];
