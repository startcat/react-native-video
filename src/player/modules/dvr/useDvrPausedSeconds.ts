/*
 *  HOOK useDvrPausedSeconds
 *  Calculamos el tiempo que hemos detenido el directo en DVR para calcular la posición en la ventana de tiempo
 *
 */
...
export interface useDvrPausedSecondsResults {
...
export function useDvrPausedSeconds(props: useDvrPausedSecondsProps) {
    ...
    const resultsRef = useRef<useDvrPausedSecondsResults>({
        pausedSeconds: 0,
        pausedDatum: 0
    });
    ...
    const updateResults = useCallback((newResults: useDvrPausedSecondsResults) => {
