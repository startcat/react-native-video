...
const addLiveTimestamp = (
	...
	if (isLiveProgramStartingPoint) {
		...
		log(
			`DVR fromDate ${fromDate.toLocaleTimeString()} from timestamp ${liveStartProgramTimestamp}`
		);
	} else if (subtractMinutes) {
		fromDate = subtractMinutesFromDate(fromDate, subtractMinutes);
		log(
			`DVR fromDate ${fromDate.toLocaleTimeString()} from subtractMinutes ${subtractMinutes}`
		);
	}
...
export const getBestManifest = (
	...
	if (manifest?.manifestURL) {
		manifest.manifestURL = getAbsoluteUri(manifest.manifestURL);
	}

	if (manifest?.drmConfig?.certificateURL) {
		manifest.drmConfig.certificateURL = getAbsoluteUri(manifest.drmConfig.certificateURL);
	}

	if (manifest?.drmConfig?.licenseAcquisitionURL) {
		manifest.drmConfig.licenseAcquisitionURL = getAbsoluteUri(
			manifest.drmConfig.licenseAcquisitionURL
		);
	}
...
export const getVideoSourceUri = (
	manifest: IManifest,
