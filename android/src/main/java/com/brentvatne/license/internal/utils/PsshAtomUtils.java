...
import com.brentvatne.license.internal.model.Atom;
...
final class PsshAtomUtils {
    ...
    static byte[] buildPsshAtom(UUID[] keyIds, @SuppressWarnings("SameParameterValue") byte[] data) {
        ...
        psshBox.putLong(C.COMMON_PSSH_UUID.getMostSignificantBits());
        psshBox.putLong(C.COMMON_PSSH_UUID.getLeastSignificantBits());
        if (buildV1Atom) {
